...
Реальный цикл из двух прогонов `jmlc-adaption-council` по этому репозиторию (находки →
исправления → повторная проверка) лежит в
[`example_council_review.md`](example_council_review.md):

- прогон 1 нашел 3 реальные проблемы (overlap-проверка заявлена в статье, но не реализована; `calibrate_stacked_scores` — не калибровка, а ансамбль; рассинхрон «конфигурации по умолчанию») и 1 ложную (отсеяна сверкой с `git ls-files`);
- после исправлений прогон 2 дал `Decision: ready`, закрытие проверено исполнением (воспроизведены числа, overlap AUC≈0.49, Platt реально меняет скоры).
