...
- прогон 1 нашёл 3 реальные проблемы (overlap-проверка заявлена в статье, но не реализована; `calibrate_stacked_scores` — не калибровка, а ансамбль; рассинхрон «конфигурации по умолчанию») и 1 ложную (отсеяна сверкой с `git ls-files`);
